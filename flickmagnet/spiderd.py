	...
	for r in dbc.fetchall():
		...
		for search_url in season_search_urls:
			...
			# python2
				...
				results_response = requests_session.get(search_url % (urllib.quote(r['series_name']), r['season_number']))
...
""" % (
	...
	r['season_id']
			...
			if magnets_found > 0:
				...
				# get some episode info
				crawl_imdb_title(settings, db, requests_session, r['id']);
